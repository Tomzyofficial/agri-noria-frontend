...
export function FeaturedVendorCard({ vendor, campaignId, href = "#", track = true }) {
   ...
   const name = vendor ? `${vendor.fname || ""} ${vendor.lname || ""}`.trim() : "Vendor";
      ...
      >
         <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
            {vendor?.profile_image_url ? (
               ...
               <img src={vendor.profile_image_url} alt={name} className="h-full w-full object-cover" />
            ) : (
         ...
         <div className="min-w-0 flex-1">
            ...
            {vendor?.is_verified ? (
               <p className="text-xs text-emerald-600 dark:text-emerald-400">Verified vendor</p>
            ) : null}
         </div>
      </a>
   );
