...
function statusPill(status) {
  const map = {
    ...
    CANCELLED:
      "bg-rose-500/10 text-rose-800 ring-rose-500/25 dark:text-rose-100",
  };
  const cls = map[status] || map.DRAFT;
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ring-1 ${cls}`}
    >
      {status}
    </span>
  );
  ...
  if (!campaigns?.length) {
    return (
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50/80 p-10 text-center text-sm text-slate-600 dark:border-slate-600 dark:bg-slate-900/40 dark:text-slate-300">
        No campaigns yet. Create one to promote listings, your vendor profile,
        or trainings.
      </div>
    );
  }
