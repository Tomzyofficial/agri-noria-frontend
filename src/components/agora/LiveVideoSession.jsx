...
import { AlertCircle, Camera, CameraOff, Loader2, Mic, MicOff, PhoneOff, Radio, ScreenShare, Settings, Users } from "lucide-react";
...
export function LiveVideoSession({ sessionData, onEndSession }) {
   ...
   const getRemoteVideoId = (uid) => `remote-video-${String(uid).replace(/[^a-zA-Z0-9_-]/g, "-")}`;
   ...
   const initializeAgora = async () => {
      try {
            ...
            try {
               ...
               if (mediaType === "video") {
                  setRemoteUsers((prev) => {
                     ...
                     return exists ? prev.map((existingUser) => (existingUser.uid === user.uid ? user : existingUser)) : [...prev, user];
                  });
                  playRemoteVideo(user);
               }
         ...
         const [localAudioTrack, localVideoTrack] = await Promise.all([AgoraRTC.createMicrophoneAudioTrack(), AgoraRTC.createCameraVideoTrack()]);
         ...
         <main className="flex min-h-screen items-center justify-center bg-slate-950 text-white">
            <div className="flex flex-col items-center gap-4 rounded-lg border border-white/10 bg-white/[0.03] px-8 py-7 shadow-2xl">
               ...
               <div className="text-center">
                  <p className="text-base font-semibold">Connecting to live training</p>
                  <p className="mt-1 text-sm text-slate-400">Setting up your camera, audio, and secure room token.</p>
               </div>
            </div>
         </main>
      );
      ...
      return (
         <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-white">
            <section className="w-full max-w-md rounded-lg border border-red-400/20 bg-red-950/20 p-6 shadow-2xl">
               ...
               <Button type="button" onClick={onEndSession} className="mt-6 w-full rounded bg-white px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-slate-100">
                  Return to trainings
               </Button>
            </section>
         </main>
      ...
      <main className="min-h-screen bg-slate-950 text-white">
         <header className="flex min-h-16 flex-col gap-3 border-b border-white/10 bg-slate-950/95 px-4 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-6">
            <div className="min-w-0">
               ...
               <h1 className="mt-1 truncate text-lg font-semibold text-white">{sessionData.trainingTitle || "Training Session"}</h1>
            </div>
         ...
         <section className="grid min-h-[calc(100vh-9rem)] gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_21rem] lg:p-6">
            <div className="grid gap-4">
               <div className="relative min-h-[22rem] overflow-hidden rounded-lg border border-white/10 bg-slate-900 shadow-2xl">
                  <div ref={localVideoRef} className="h-full min-h-[22rem] w-full bg-slate-900 [&_*]:!h-full [&_*]:!w-full [&_video]:!h-full [&_video]:!w-full [&_video]:!object-cover" />
                  {!cameraOn && (
                     <div className="absolute inset-0 flex items-center justify-center bg-slate-900">
                        <div className="flex h-24 w-24 items-center justify-center rounded-full bg-slate-800 text-2xl font-semibold text-slate-200">{roleLabel.slice(0, 1)}</div>
                     </div>
                  )}
                  <div className="absolute left-4 top-4 rounded border border-black/20 bg-black/55 px-3 py-1.5 text-xs font-medium text-white backdrop-blur">{roleLabel}</div>
                  <div className="absolute bottom-4 left-4 flex items-center gap-2 rounded border border-black/20 bg-black/55 px-3 py-1.5 text-xs text-slate-100 backdrop-blur">
                     {micOn ? <Mic className="h-4 w-4" /> : <MicOff className="h-4 w-4 text-red-300" />}
                     {cameraOn ? <Camera className="h-4 w-4" /> : <CameraOff className="h-4 w-4 text-red-300" />}
                     You
                  </div>
               </div>
               ...
               <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                  {remoteUsers.length > 0 ? (
                     remoteUsers.map((user) => (
                        <div key={user.uid} id={getRemoteVideoId(user.uid)} className="relative aspect-video overflow-hidden rounded-lg border border-white/10 bg-slate-900 [&_*]:!h-full [&_*]:!w-full [&_video]:!h-full [&_video]:!w-full [&_video]:!object-cover">
                           {!user.videoTrack && (
                              <div className="absolute inset-0 flex items-center justify-center bg-slate-900 z-10">
                                 <div className="flex h-16 w-16 items-center justify-center rounded-full bg-slate-800 text-xl font-semibold text-slate-200">{formatUidToName(user.uid).charAt(0)}</div>
                              </div>
                           )}
                           <span className="absolute bottom-2 left-2 rounded bg-black/55 px-2 py-1 text-xs text-white z-20">{formatUidToName(user.uid)}</span>
                        </div>
                     ))
                  ) : (
                     <div className="flex aspect-video items-center justify-center rounded-lg border border-dashed border-white/15 bg-white/[0.03] text-sm text-slate-400 sm:col-span-2 xl:col-span-3">Waiting for participants to join</div>
                  )}
               </div>
            </div>
            ...
            <aside className="rounded-lg border border-white/10 bg-white/[0.04] p-4">
               <div className="flex items-center justify-between border-b border-white/10 pb-4">
                  ...
                  <span className="rounded bg-emerald-400/10 px-2.5 py-1 text-xs font-medium text-emerald-200">{roleLabel}</span>
               </div>

               <dl className="mt-4 space-y-4 text-sm">
                  <div>
                     <dt className="text-slate-500">Channel</dt>
                     <dd className="mt-1 break-all font-medium text-slate-200">{sessionData.channelName}</dd>
                  </div>
                  <div>
                     <dt className="text-slate-500">Connection UID</dt>
                     <dd className="mt-1 break-all font-medium text-slate-200">{sessionData.uid}</dd>
                  </div>
                  <div>
               ...
               <div className="mt-6 rounded-lg border border-amber-300/20 bg-amber-300/10 p-3 text-xs leading-5 text-amber-50/80">Keep this tab open while the live session is active. Opening the same room in multiple tabs can reserve multiple seats.</div>
            </aside>
         </section>
         ...
         <footer className="sticky bottom-0 border-t border-white/10 bg-slate-950/95 px-4 py-4 backdrop-blur lg:px-6">
            <div className="mx-auto flex max-w-4xl items-center justify-center gap-3">
               <ControlButton active={micOn} label={micOn ? "Mute microphone" : "Unmute microphone"} onClick={toggleMic}>
                  {micOn ? <Mic className="h-5 w-5" /> : <MicOff className="h-5 w-5" />}
               </ControlButton>

               <ControlButton active={cameraOn} label={cameraOn ? "Turn camera off" : "Turn camera on"} onClick={toggleCamera}>
               ...
               <Button type="button" onClick={handleLeave} className="cursor-pointer inline-flex h-11 items-center gap-2 rounded bg-red-600 px-4 text-sm font-semibold text-white hover:bg-red-700">
...
function ControlButton({ active = false, disabled = false, label, onClick, children }) {
   return (
      <Button
         type="button"
         disabled={disabled}
         ...
         title={label}
         onClick={onClick}
         className={`inline-flex h-11 w-11 items-center justify-center rounded border text-white transition ${active ? "border-white/10 bg-white/10 hover:bg-white/15" : "border-red-400/30 bg-red-500/20 text-red-100 hover:bg-red-500/30"} ${disabled ? "cursor-not-allowed opacity-40" : "cursor-pointer"}`}
      >
         {children}
      </Button>
   );
