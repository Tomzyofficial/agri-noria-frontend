...
export async function signinBridge(credentials) {
   try {
      ...
      if (!res.ok || !data.success) {
         ...
         console.log("Vendor sign-in failed with status:", status, "and error:", data.error);
...
export async function signoutBridge() {
   ...
   const backend = await fetch(apiUrl("/api/auth/vendor/signout"), { method: "POST" });
...
export async function buyerSignoutBridge() {
   ...
   const backend = await fetch(apiUrl("/api/auth/buyer/signout"), { method: "POST" });
   if (!backend.ok) {
      return false;
   }
   return true;
