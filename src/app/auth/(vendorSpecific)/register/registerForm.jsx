...
import { registerFormSchema } from "@/_lib/definitions";
...
const roleCategories = [
   {
      name: "Institution",
      roles: ["Government", "Bank", "NGO", "DFI", "Insurance Firm", "Commodity Board", "Finance", "Distributor"],
   },
   {
...
export function RegisterForm() {
   ...
   const validateWithSchemaForStep = (step) => {
      ...
      if (step === 1) {
         schemaForStep = registerFormSchema.pick({ fname: true, lname: true, email: true });
      } else if (step === 2) {
   ...
   const handleSubmit = async (e) => {
      ...
      try {
         ...
         setErrors((prev) => ({ ...prev, general: "Internal server error. Please try again." }));
         return;
      } finally {
         setIsLoading(false);
      }
                        ...
                        <div className="grid md:grid-cols-2 gap-4">
                           ...
                           <div>
                              ...
                              <Input
                                 id="account_type"
                                 type="text"
                                 className={isLoading ? "opacity-50 bg-gray-100" : "bg-gray-100 cursor-not-allowed"}
                                 disabled={true}
                                 name="account_type"
                           ...
                           <div className="hidden">
                              <input
                                 type="hidden"
                                 name="country_name"
                                 value={Country.getCountryByCode(formData.country_code)?.name || ""}
                                 readOnly
                              />
                              <input
                                 type="hidden"
                              ...
                              <input
                                 type="hidden"
                                 name="currency"
                                 value={Country.getCountryByCode(formData.country_code)?.currency || ""}
                                 readOnly
                              />
                           </div>
                        </div>
                        ...
                        <div>
                           ...
                           {errors.err?.terms_of_service && (
                              <p className="text-start text-red-400 dark:text-red-300 text-sm mb-5">
                                 {errors.err.terms_of_service[0] || errors.err.terms_of_service}
                              </p>
                           )}
                        </div>
                     )}
