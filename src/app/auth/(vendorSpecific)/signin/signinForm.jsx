  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (session.authenticated) {
        ...
        //     const isMarketplace = ["seller", "logistics", "storage facility", "trainer", "farmer", "farm-development"].includes(normalizedRole);
        //     normalizedWorkspace = isMarketplace ? "marketplace" : "ecosystem";
        //   }

        const categoryRoute = roleConfig[normalizedRole]?.category?.toLowerCase()?.replace(/ \/ /g, "-")?.replace(/ /g, "-");
        ...
        if (normalizedRole === "super admin" || normalizedRole === "admin") {
        ...
        } else if (normalizedWorkspace === "marketplace" && ["farmer", "seller"].includes(normalizedRole)) {
          ...
          router.push(`/${normalizedWorkspace}/${normalizedRole?.replace(/\s+/g, "-") || ""}`);
        }
      }
      router.refresh();
    } catch (error) {
