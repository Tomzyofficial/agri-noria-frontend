  ...
  return (
    <div className="bg-background dark:bg-(--card-dark) rounded-lg shadow-sm overflow-hidden sticky top-96">
      <div className="p-6 space-y-4">
        ...
        <button type="button" onClick={() => setOpenModal(true)} disabled={vehicle.status !== "available"} className={`w-full py-3 px-4 rounded-lg font-semibold text-white transition-all duration-200 ${vehicle.status !== "available" ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700 active:scale-95"}`}>
        ...
        <button onClick={handleShare} className="w-full py-3 px-4 rounded-lg font-semibold text-green-700 border-2 border-green-600 hover:bg-green-50 active:scale-95 transition-all duration-200">
          Share This Vehicle
        </button>
        ...
        <div className="bg-blue-50 rounded-lg p-3 mt-4 border border-blue-100">
          <p className="text-xs text-blue-900">
            <strong>💡 Tip:</strong> Compare pricing and capacity with other vehicles to find the best match for your needs.
          </p>
        </div>
      </div>
      {/* Quote request form */}
      <Modal onClick={() => setOpenModal(false)} isOpen={openModal}>
        <LogisticsForm onClose={() => setOpenModal(false)} targetId={vehicle} quoteType="logistics" />
      </Modal>
    </div>
  );
}
