...
export function TrainingPage({ materials, error }) {
  ...
  const filteredMaterials = materials.filter((material) => {
    const matchesType = typeFilter === "all" || material.file_type?.toLowerCase().includes(typeFilter);

    const matchesCategory = categoryFilter === "all" || material.category === categoryFilter;

    const matchesSearch = !searchQuery || material.title?.toLowerCase().includes(searchQuery.toLowerCase()) || material.description?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden" style={{ background: "#2D4A3E" }}>
        <div className="absolute -top-20 -right-20 w-80 h-80 rounded-full opacity-10" style={{ background: "#8CC9AE" }} />
        <div className="absolute -bottom-10 -left-10 w-48 h-48 rounded-full opacity-10" style={{ background: "#8CC9AE" }} />
        <div className="relative max-w-7xl mx-auto px-6 py-16">
          ...
          <p className="text-base max-w-lg mb-10" style={{ color: "rgba(255,255,255,0.6)", lineHeight: 1.7 }}>
            Browse curated training materials — videos, guides, and visual references — sourced from our certified vendor partners.
          </p>
      ...
      <div className="sticky top-20 z-40 border-b border-stone-200 bg-white/95 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 py-3 flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px] max-w-xs">
            ...
            <Input type="text" ref={searchTimerRef} value={searchInput} onChange={handleSearchChange} placeholder="Search materials…" className="w-full pl-9 pr-8 py-2 text-sm bg-stone-50 border border-stone-200 rounded-full outline-none transition-all focus:border-emerald-400 focus:ring-2 focus:bg-white" />
            {searchInput && (
          ...
          <div className="flex items-center gap-1 bg-stone-100 p-1 rounded-full">
            {FILE_TYPES.map(({ value, label, Icon }) => (
              <Button key={value} onClick={() => setTypeFilter(value)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium transition-all" style={typeFilter === value ? { background: "#2D4A3E", color: "#fff" } : { color: "#78716c" }}>
                {Icon && <Icon size={13} />}
                {label}
              </Button>
            ))}
          ...
          {hasActiveFilters && (
            <Button onClick={clearAllFilters} className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full transition-colors hover:opacity-80" style={{ background: "#FFF3E8", color: "#B84A2E" }}>
              <X size={12} />
              Clear filters
            </Button>
          )}
      ...
      <footer className="border-t border-stone-200 mt-10">
        <div className="max-w-7xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-3">
          <span className="text-sm text-stone-400">© {new Date().getFullYear()} Training Hub. Agri-Noria. All rights reserved.</span>
