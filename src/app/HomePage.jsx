            ...
            {menu.map((item, index) => (
              <li
                ...
                className={`relative group bg-transparent lg:bg-(--white-fff) lg:dark:bg-(--card-dark) lg:shadow-lg lg:p-1 lg:rounded lg:w-full ${
                  index === 0
                    ? "lg:mb-4 lg:border-b-2 lg:border-blue-500/20 pb-2"
                    : ""
                }`}
              >
                <Link
                ...
                >
                  <span className="flex items-center gap-2 lg:flex-row flex-col text-center lg:text-left">
                    <span
                      className={`lg:bg-transparent lg:p-0 p-4 rounded-xl ${index === 0 ? "bg-blue-100 dark:bg-blue-900/30" : "bg-slate-200"}`}
                    >
                      {item.icon}
                    </span>
                  ...
                  {index === 0 && (
                    <span className="hidden lg:block text-[8px] bg-blue-600 text-white px-1 rounded ml-2 uppercase tracking-tighter">
                      Enter
                    </span>
                  )}
                </Link>
                  ...
                  <div className="rounded-lg bg-(--gray-color) shadow hover:shadow-lg h-70 transition dark:bg-(--card-dark)">
                    ...
                    <div className="p-2 space-y-2">
                      ...
                      <p className="font-semibold">
                        {prod.listing_name
                          ? prod.listing_name.charAt(0).toUpperCase() +
                            prod.listing_name.slice(1)
                          : ""}
                      </p>
