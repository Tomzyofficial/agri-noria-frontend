         ...
         <section>
            <div className="relative min-h-[250px] bg-(--greenish-color) rounded-bl-2xl rounded-br-2xl flex justify-center items-center">
               <div className="w-90 sm:w-100 md:w-1/2">
                  <p className="text-(--white-fff) dark:text-[#171717] font-medium text-md md:text-lg mb-5 ml-5">What are you looking for?</p>
                  <div className="relative flex items-center">
                     <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4" />
                     <Input value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="I am looking for..." className="bg-(--background) rounded-md pl-10 py-3 focus:outline-none shadow-md w-full" type="text" />
                  </div>
               </div>
            </div>
         </section>
                     ...
                     filteredProducts.map((prod) => (
                        <Link href={`/products/${prod.id}`} key={prod.id}>
                           <div className="rounded-lg bg-(--gray-color) shadow hover:shadow-lg h-70 transition dark:bg-(--card-dark)">
                              <Image priority src={prod.product_image} alt={`${prod.listing_name} Image`} width={300} height={200} className="object-cover shrink-0 w-full h-40 rounded-t-lg" />
                              <div className="p-2 space-y-2">
                                 <p className="text-green-700 font-bold">{formatPrice(prod.price, prod.country_code, prod.currency)}</p>
