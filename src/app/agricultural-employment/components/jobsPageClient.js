...
export function JobsPageClient({ jobs }) {
   ...
   return (
      <>
         ...
         <section>
            <div className="flex items-center gap-4 p-2 h-15 overflow-y-auto">
               <span className="inline-flex items-center">
                  <IoIosFunnel />
                  TYPE
               </span>
               {JOBTYPE.map((type) => (
                  <Button key={type} className={`w-full border text-foreground border-gray-500 px-4 py-1 rounded-full ${typeFilter === type ? "bg-[#1a481e] text-white" : "bg-gray-200 dark:bg-(--card-dark)"}`} onClick={() => setTypeFilter(type)}>
                     {type}
                  </Button>
               ))}
            </div>
         ...
         <main className="overflow-x-hidden">
            <div>
               {jobs.length === 0 && !typeFilter ? (
                  ...
                  <div className="flex h-[calc(100vh-80px)] text-foreground gap-3 w-[100%] mt-10 mx-2">
                     <aside className="w-[35%] overflow-y-auto divide-y divide-gray-300">
                        {filtered.map((job) => (
                           <div key={job.id} className={`${selectedJob?.id === job.id ? "bg-gray-200 dark:bg-(--card-dark) border border-gray-300 rounded-lg" : ""} p-2 hover:bg-gray-200 dark:hover:bg-(--card-dark)`}>
                     ...
                     {/* right side */}
                     <section className="overflow-y-auto bg-gray-100 dark:bg-(--card-dark) p-2 w-[65%]">
                        {selectedJob && (
                           <>
                              <div className="sticky top-0 bg-gray-200 dark:bg-gray-600 rounded-lg p-4">
