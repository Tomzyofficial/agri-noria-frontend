...
export default function QuoteModal({ provider, service, onClose }) {
  ...
  const handleSubmit = async () => {
    ...
    setErrors({});
    try {
      const res = await fetch('/api/proxy/farming/quotes', {
        method: 'POST',
  ...
  return (
    <div style={{
      position: 'fixed', inset: 0, background: 'rgba(44,26,14,0.65)', zIndex: 1000,
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <h2 style={{ fontFamily: 'Playfair Display, serif', fontSize: '22px', color: '#2C1A0E' }}>
              Request a Quote
            </h2>
            <p style={{ fontSize: '13px', color: '#8B5E3C', marginTop: '2px' }}>
              {provider.name}{service ? ` · ${service.title}` : ''}
            </p>
          </div>
          <button onClick={onClose} style={{
            width: '32px', height: '32px', borderRadius: '50%', border: '1px solid #E0D4C4',
            background: 'transparent', cursor: 'pointer', fontSize: '18px', color: '#8B5E3C',
        ...
        {success ? (
          <div style={{ padding: '48px 28px', textAlign: 'center' }}>
            ...
            <h3 style={{ fontFamily: 'Playfair Display, serif', fontSize: '22px', color: '#2C1A0E', marginBottom: '8px' }}>
              Quote Request Sent!
            </h3>
            ...
            <button onClick={onClose} style={{
              background: '#3D6B45', color: '#fff', border: 'none', borderRadius: '10px',
          ...
          <div style={{ padding: '24px 28px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              ...
              <div>
                ...
                <input name="client_name" value={form.client_name} onChange={handleChange} style={inputStyle('client_name')} placeholder="Your full name" />
                {errors.client_name && <p style={{ color: '#E53E3E', fontSize: '11px', marginTop: '4px' }}>{errors.client_name}</p>}
              </div>
              {/* Email */}
              <div>
              ...
              <div>
                ...
                <input name="client_phone" value={form.client_phone} onChange={handleChange} style={inputStyle('client_phone')} placeholder="+234 800 000 0000" />
              </div>
              {/* Organization */}
              <div>
            ...
            <div style={{ marginTop: '14px' }}>
              ...
              <textarea name="project_description" value={form.project_description} onChange={handleChange}
                style={{ ...inputStyle('project_description'), minHeight: '100px', resize: 'vertical' }}
                placeholder="Describe your project — what crops, type of services needed, land status, goals..." />
              {errors.project_description && <p style={{ color: '#E53E3E', fontSize: '11px', marginTop: '4px' }}>{errors.project_description}</p>}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginTop: '14px' }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input name="land_size" type="number" value={form.land_size} onChange={handleChange}
                    style={{ ...inputStyle('land_size'), flex: 1 }} placeholder="e.g. 500" />
                  <select name="land_size_unit" value={form.land_size_unit} onChange={handleChange}
                    style={{ ...inputStyle('land_size_unit'), width: 'auto', paddingRight: '8px' }}>
            ...
            <div style={{ marginTop: '20px', display: 'flex', gap: '12px' }}>
              <button onClick={onClose} style={{
                flex: 1, padding: '13px', border: '1px solid #D6C9B8', borderRadius: '10px',
                background: 'transparent', color: '#5C3D1E', fontWeight: '600', cursor: 'pointer', fontSize: '14px',
              }}>Cancel</button>
