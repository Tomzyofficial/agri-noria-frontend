...
export function ViewItem({ storage }) {
    ...
    return (
      <div className="py-12 text-center">
        ...
        <p className="text-stone-500 dark:text-stone-400 text-sm mt-2">
          The storage facility you're looking for doesn't exist or has been
          removed.
        </p>
      </div>
    );
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        "Are you sure you want to delete this storage facility? This action cannot be undone.",
      )
    ) {
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/proxy/vendor/storage/delete-item/${storage.id}`,
        {
          method: "DELETE",
        },
      );
    ...
    <div className="py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-3">
          <Link
            href={`/marketplace/storage-facility/storage-facilities/edit-item/${storage.id}`}
          >
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border-t border-stone-200 dark:border-stone-700">
                ...
                <DetailItem
                  ...
                  value={
                    storage.available
                      ? `${storage.available} sq ft available`
                      : "Not specified"
                  }
                />
                <DetailItem
