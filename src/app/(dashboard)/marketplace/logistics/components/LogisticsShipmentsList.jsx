...
export function LogisticsShipmentsList() {
   ...
   const { data, error, isLoading, mutate } = useSWR("/api/proxy/vendor/logistics/shipments", fetcher);
   ...
   const handleOTPVerify = async (otp) => {
      ...
      try {
         const res = await fetch(`/api/proxy/vendor/logistics/orders/${completeDeliveryOrderId}/complete-delivery`, {
            method: "POST",
            headers: {
   ...
   return (
      <div className="space-y-6">
         <div>
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">Accepted orders ready for pickup and delivery</p>
         </div>

         <div className="bg-white dark:bg-(--card-dark) rounded-xl shadow-sm border overflow-hidden">
            {error && <div className="p-6 text-red-600 text-sm">{error.message || "Failed to load shipments"}</div>}
            {isLoading && <div className="p-10 text-center text-gray-500">Loading shipments...</div>}
            {!isLoading && !error && shipments.length === 0 && (
               <div className="p-10 text-center text-gray-500">
                  No active shipments. Accept paid orders from the{" "}
                  <Link href="/marketplace/logistics/orders?status=paid" className="text-green-700 underline">
                     orders page
                  </Link>
                  .
               </div>
                              ...
                              <td className="px-4 py-4">
                                 <div className="flex flex-wrap gap-2">
                                    ...
                                    {order.status === "in_transit" && (
                                       <button type="button" onClick={() => handleCompleteDelivery(order.id)} className="inline-flex items-center gap-1 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded">
