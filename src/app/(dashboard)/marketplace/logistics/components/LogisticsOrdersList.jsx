...
import { ORDER_STATUS_CONFIG, getStatusBadgeClass, formatStatusLabel } from "./logisticsOrderUtils";
...
export function LogisticsOrdersList() {
  ...
  const handleDecline = async (orderId) => {
    ...
    try {
      ...
      toast.success(data.message || "Order declined");
      mutate();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <Link href="/marketplace/logistics" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-800 mb-2">
            <ArrowLeft className="w-4 h-4" />
            Back to overview
          </Link>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Assigned orders</h1>
      ...
      <div className="flex flex-wrap gap-2">
        <Button type="button" onClick={() => router.push("/marketplace/logistics/orders")} className={`px-3 cursor-pointer py-1.5 rounded-full text-sm border ${!statusFilter ? "bg-green-100 border-green-300 text-green-800" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
          All
        </Button>
        {ORDER_STATUS_CONFIG.map(({ status, icon, label }) => (
          <Link key={status} href={`/marketplace/logistics/orders?status=${status}`} className={`px-2 flex items-center gap-2 py-1.5 rounded-full text-sm border ${statusFilter === status ? "bg-green-100 border-green-300 text-green-800" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
