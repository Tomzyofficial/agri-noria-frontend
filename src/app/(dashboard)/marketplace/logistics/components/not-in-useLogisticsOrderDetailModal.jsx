      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
         <div className="bg-white dark:bg-(--card-dark) rounded-xl shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-6">
               {loading && <p className="text-center text-gray-500 py-8">Loading order...</p>}
               {error && <p className="text-center text-red-600 py-8">{error}</p>}

               {detail && !loading && (
                  <>
                     <div className="flex flex-wrap items-center gap-3">
                        <span className="font-mono text-sm text-gray-600">{detail.id}</span>
                        <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusBadgeClass(detail.status)}`}>{formatLabel(detail.status)}</span>
                     </div>
                     ...
                     <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <section className="rounded-lg border p-4 space-y-3">
                           ...
                           <p className="text-sm">
                              <span className="text-gray-500">Name: </span>
                              {[detail.seller_fname, detail.seller_lname].filter(Boolean).join(" ") || detail.seller_business_name || "—"}
                           </p>
                           <p className="text-sm flex items-center gap-1">
                              <Phone className="w-3.5 h-3.5 text-gray-400" />
                              {detail.seller_phone || "—"}
                                    ...
                                    <tr key={item.product_id}>
                                       <td className="px-3 py-3">
                                          <div className="flex items-center gap-3">
                                             {item.product_image && <img src={item.product_image} alt={item.product_name || "Product"} width={40} height={40} className="rounded object-cover w-10 h-10" />}
                                             <span>{item.product_name}</span>
                                          </div>
                                       </td>
                                       <td className="px-3 py-3">{item.quantity}</td>
                                       <td className="px-3 py-3">₦{Number(item.unit_price).toLocaleString()}</td>
                                       <td className="px-3 py-3">₦{(Number(item.unit_price) * Number(item.quantity)).toLocaleString()}</td>
                                    </tr>
                                 ))}
                              </tbody>
                           </table>
                     ...
                     <div className="flex justify-end gap-2 text-sm text-gray-600">
                        ...
                        <span>·</span>
                        <span className="font-semibold text-gray-900 dark:text-white">Total: ₦{Number(detail.total_amount || 0).toLocaleString()}</span>
                     </div>
                  </>
               )}
            </div>
