...
import { LogOut, X, Menu, LayoutDashboard, CreditCard, Play, UserPen, Calendar, Megaphone } from "lucide-react";
...
export default function DashboardLayout({ children }) {
  ...
  const sidebarNavMenu = [
    ...
    },
    {
      ...
      href: "/marketplace/trainer/upload-material",
      icon: <Play className="w-4 h-4" />,
    },
    {
      label: "Profile",
      href: "/marketplace/trainer/profile",
  ...
  const navLinksStyle = (path) => {
    const active = path === pathname || (path === "/marketplace/ads" && pathname.startsWith("/marketplace/ads"));
    return active ? "bg-gray-200 dark:bg-(--card-dark) p-2 rounded" : "hover:bg-gray-200 dark:hover:bg-(--card-dark) dark:focus:bg-(--card-dark) p-2 focus:bg-gray-200 rounded transition delay-50 duration-150 ease-in-out";
  };
  ...
  return (
    <div className="flex">
      ...
      <aside>
        ...
        <div className={`${menuOpen && "left-0 w-64 h-full bg-(--gray-color) shadow shadow-md"} transition-all duration-300 fixed z-1 -left-64 top-0 lg:left-0 lg:w-64 lg:h-screen dark:bg-(--card-dark) dark:text-(--foreground) lg:bg-(--gray-color) lg:shadow lg:shadow-md p-2`}>
          <nav
            className={`
