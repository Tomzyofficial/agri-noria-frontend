...
export function ProductManagement() {
  ...
  const { data: productsData, mutate, error: productsError, isLoading: productsLoading } = useSWR("/api/proxy/vendor/products/listed", fetcher);
  ...
  const handleDelete = async (productId) => {
    ...
    try {
      const res = await fetch(`/api/proxy/vendor/products/delete-item/${productId}`, {
        method: "DELETE",
      });
              ...
              filteredProducts.map((product) => (
                <Card key={product.id} className="max-w-2xl min-h-70">
                  <div className="aspect-square rounded-t-lg ">
                    <Image src={product.product_image} alt={`${product.listing_name} Image`} width={500} priority sizes="(max-width: 1200px) 100vw" height={500} className="rounded-t-lg w-full h-full object-cover" />
                  </div>
