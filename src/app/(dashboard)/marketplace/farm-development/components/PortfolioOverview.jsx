...
import { PortfolioListingsCard } from "@/app/(dashboard)/marketplace/farm-development/components/PortfolioListingsCard";
        ...
        filteredProjects.length > 0 && (
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-2 ">
            {filteredProjects.map((listing) => (
              <PortfolioListingsCard
                key={listing.id}
                id={listing.id}
                title={listing.title}
                src={listing.featured_image}
                url={`/api/proxy/farm-development/portfolio/delete/${listing.id}`}
                description={listing.description}
                viewHref={`/marketplace/farm-development/portfolio/${listing.id}/view`}
                editHref={`/marketplace/farm-development/portfolio/${listing.id}/edit`}
