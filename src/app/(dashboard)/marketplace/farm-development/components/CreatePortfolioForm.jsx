...
export default function CreatePortfolioPage() {
   ...
   return (
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white dark:bg-(--card-dark) p-6 rounded-lg shadow">
         <h1 className="text-2xl font-bold mb-6">Create Farm Development Portfolio</h1>

         {/* STEP INDICATOR */}
         <div className="flex gap-2 mb-6 text-sm">
            {[1, 2, 3].map((s) => (
               <div key={s} className={`px-3 py-1 bg-gray-200 text-gray-700 rounded-full ${currentStep === s && "bg-green-600 text-white"}`}>
                  Step {s}
               </div>
            ))}
         </div>
         ...
         {currentStep === 3 && (
            <section className="space-y-6">
               <div>
                  ...
                  <Input type="file" accept="image/.png, .jpg, .jpeg, .webp" onChange={handleFeaturedImage} />
              ...
              <p className="text-sm flex items-center text-green-600 mt-1">
               ...
               <div>
                  ...
                  <Input type="file" accept="image/.png, .jpg, .jpeg, .webp" multiple onChange={handleGalleryImages} />
