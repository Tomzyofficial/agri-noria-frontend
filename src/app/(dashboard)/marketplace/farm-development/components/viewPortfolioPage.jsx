...
import { Edit, Trash2, ZoomIn, Calendar, CheckCircle2, Clock, Info, MapPin, Wallet, Users, Star, Eye } from "lucide-react";
  ...
  if (!project) {
    return (
      <div className="py-12 text-center">
        ...
        <p className="text-stone-500 dark:text-stone-400 text-sm mt-2">The project you&apos;re looking for doesn&apos;t exist or has been removed.</p>
    ...
    <div className="py-6 space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-stone-500 dark:text-stone-400 mt-1">View and manage your completed farm development project</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <Card className="overflow-hidden">
            <CardContent className="p-0">
              <div onClick={() => openImageModal(project.featured_image)} className="relative w-full h-[300px] sm:h-[400px] bg-stone-100 dark:bg-stone-800 group cursor-zoom-in">
                <Image src={project.featured_image} fill sizes="(max-width: 768px) 100vw, 33vw" alt={`${project.title} featured image`} className="object-cover transition-transform duration-300 group-hover:scale-105" priority />
              ...
              {galleryImages.length > 0 && (
                <div className="flex gap-3 overflow-x-auto p-4 border-t border-stone-200 dark:border-stone-700">
                  {galleryImages.map((src) => (
                    <Button key={src} type="button" onClick={() => openImageModal(src)} className="relative shrink-0 rounded-md overflow-hidden ring-2 ring-transparent hover:ring-(--greenish-color) transition-all cursor-zoom-in">
                      <Image src={src} width={120} height={120} alt={`Gallery image for ${project.title}`} className="w-[120px] h-[120px] object-cover" />
                    </Button>
                  ))}
                </div>
              )}
        ...
        <div className="space-y-6">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 text-start gap-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${project.status === "active" ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400"}`}>
          ...
          <Card className="p-2">
            ...
            <CardContent className="space-y-4">
              <DetailItem icon={<Calendar className="h-4 w-4" />} label="Completion Date" value={project.completion_date ? formatDate(project.completion_date) : "Not specified"} />
      ...
      {isModalOpen && activeImage && <ImageEnlargementModal isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} name={`${project.title} - Enlarged Image View`} src={activeImage} />}
    </div>
  );
}
...
function DetailRow({ label, value, multiline = false }) {
  return (
    <div className="pt-4 first:pt-0">
      ...
      <p className={`text-md text-(--foreground) mt-1 ${multiline ? "whitespace-pre-wrap leading-relaxed" : ""}`}>{value || "Not specified"}</p>
    </div>
  );
}
