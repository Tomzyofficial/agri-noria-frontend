...
export default function JobsPage({ createHref, role }) {
   ...
   return (
      <div className="space-y-8">
         ...
         <section className="rounded-md border border-stone-200 dark:border-stone-800 bg-(--gray-color) dark:bg-(--card-dark)">
                           ...
                           jobs.map((job) => (
                              <tr key={job.id}>
                                 ...
                                 <td className="p-4 flex items-center gap-6">
                                    <Link title="View" href={`/marketplace/${role}/job-management/${job.id}/applicants`} className="font-medium text-primary">
                                       <FaEye />
                                    </Link>
                                    <Button disabled={isDeleting === job.id} className="cursor-pointer" title="Delete" onClick={() => handleDeleteJOb(job.id)}>
                                       {isDeleting === job.id ? <FaSpinner className="animate-spin" /> : <FaTrash className="text-red-500" />}
                                    </Button>
                                 </td>
                              </tr>
                           ))}
