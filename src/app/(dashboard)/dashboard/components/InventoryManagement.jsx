...
import { NoProductsFound } from "./ui/NotFound";
                           ...
                           <Card key={product.id} className="max-w-2xl min-h-70">
                              ...
                              <div className="p-2 relative">
                                 ...
                                 <div className="text-start space-y-2">
                                    <h2 className="text-md font-semibold">{formatLabel(product.listing_name)}</h2>
                                    <p className="text-sm line-clamp-1">{formatDate(product.created_at)}</p>
                                 ...
                                 <div className={`${openActionId === product.id ? "absolute rounded right-0 top-10 bg-(--background) w-1/2 p-1 space-y-2" : "hidden"}`}>
                                    ...
                                    <Button className="w-full px-1 flex items-center gap-2 text-red-700 hover:bg-red-400 cursor-pointer transition transition-background rounded text-sm" onClick={() => handleDelete(product.id)} disabled={isDeleting[product.id]}>
            ...
            <TabsContent value="list">
               <div className="space-y-4">
                  {productsError ? (
                     ...
                     <NoProductsFound searchTerm={searchTerm} href="/marketplace/drone/inventory/add-new" />
                  ) : (
                     filteredProducts.length > 0 &&
                     filteredProducts.map((product) => {
                        const src = product.image?.[0];
                           ...
                           <Card key={product.id} className="flex justify-between px-2">
                              <div className="flex items-center gap-2">
                                 <div className="w-25 h-25 rounded-lg flex-shrink-0">
                                    <Image width={150} height={150} priority sizes="(max-width: 1200px), 100vw" src={src} alt={`${product.listing_name} Image`} className="rounded w-full h-full object-cover" />
                                 </div>

                                 <div className="flex flex-col items-center gap-1" style={{ placeItems: "start" }}>
                                    <h4 className="font-semibold text-lg">{formatLabel(product.listing_name)}</h4>
                              ...
                              <div className="relative">
                                 ...
                                 <div className={`${openActionListView === product.id ? "absolute rounded right-0 top-10 bg-(--background) w-34 p-1 space-y-2" : "hidden"}`}>
                                    ...
                                    <Button className="w-full px-1 flex items-center gap-2 hover:bg-red-400 cursor-pointer transition transition-background rounded text-red-700 text-sm" onClick={() => handleDelete(product.id)} disabled={isDeleting[product.id]}>
                                       {isDeleting[product.id] ? <FaSpinner className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                                       Delete
                                    </Button>
                                 </div>
