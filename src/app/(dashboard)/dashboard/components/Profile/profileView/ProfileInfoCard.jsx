...
export function ProfileInfoCard({ is_verified, vendor, loading, imageSrc, setOpenModal = false, handleProfileImageChange }) {
   return (
      <section className="bg-white dark:bg-(--card-dark) p-6 rounded-xl shadow-lg">
         ...
         <div className="flex justify-end">
            <Button type="button" className="cursor-pointer py-1 px-2 rounded bg-(--primary)/20 text-(--primary) dark:text-[var(--primary)] dark:bg-green-100" onClick={() => setOpenModal(true)}>
               Edit profile
            </Button>
         </div>
         ...
         <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-8">
            ...
            <div className="relative w-30 h-30 flex-shrink-0">
               {loading ? (
                  ...
                  <Image src={imageSrc} width={200} height={200} alt="Vendor profile image Preview" priority className="rounded-full w-full h-full object-cover border-4 shadow-md" />
               ) : (
               ...
               <Label htmlFor="profile_image" className="absolute bottom-0 right-0 p-2 rounded-full cursor-pointer transition duration-150 shadow-md shadow-black/40">
                  <Camera />
               </Label>
