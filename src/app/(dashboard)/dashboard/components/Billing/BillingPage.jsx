...
export default function BillingPage({ plans, user }) {
   ...
   } = useSWR(`/api/proxy/vendor/subscription/history?page=${page}&pageSize=${pageSize}`, fetcher);

   const {
      data: subscriptionData,
      error: subscriptionError,
      isLoading: subscriptionLoading,
   } = useSWR("/api/proxy/vendor/subscription/current", fetcher);
   ...
   const allCycles = Array.from(
      new Set(Array.isArray(plans) ? plans.map((p) => (p.billing_cycle === "annually" ? "annually" : "monthly")) : []),
   );
      ...
      <div>
         ...
         <div className="grid md:grid-cols-3 gap-6 mb-12">
            {Array.isArray(plans) && plans.length > 0 ? (
               plans
                  .filter((plan) =>
                     billingCycle === "monthly" ? plan.billing_cycle === "monthly" : plan.billing_cycle === "annually",
                  )
                  .map((plan, idx) => (
                     <BillingPlanCard
                        key={idx}
