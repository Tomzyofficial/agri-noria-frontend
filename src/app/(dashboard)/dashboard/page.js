...
export default async function Dashboard() {
   ...
   const isLegacyRole = ["seller", "vendor", "store", "Training_Partner"].includes(accountType);

   // Special case for Farmer: Check if they've joined the industrial ecosystem
   const isIndustrialFarmer =
      accountType === "farmer" &&
      (session.onboarding_status === "completed" || session.onboarding_status === "verified");
   ...
   if (session.onboarding_status !== "completed" && session.onboarding_status !== "verified") {
