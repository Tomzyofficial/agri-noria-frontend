...
export default function DashboardLayout({ children }) {
  ...
  const sidebarNavMenu = [
    ...
    },
    {
      ...
      href: "/dashboard/trainings/upload-material",
      icon: <Play className="w-4 h-4" />,
    },
    {
      label: "Profile",
      href: "/dashboard/trainings/profile",
      icon: <UserPen className="w-4 h-4" />,
    },
    {
      label: "Billing",
      href: "/dashboard/sub-store/billing",
  ...
  const navLinksStyle = (path) => {
    const active =
      path === pathname ||
      (path === "/dashboard/ads" && pathname.startsWith("/dashboard/ads"));
    return active
  ...
  return (
    <div className="flex min-h-screen bg-background">
      ...
      <main className="lg:ml-64 w-full lg:p-8 p-4">
        {children}
        {verified !== null && !verified && !dismissed && (
          <VerifyNotiBanner setDismissed={setDismissed} />
        )}
      </main>
    </div>
  );
