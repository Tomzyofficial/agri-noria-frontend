    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 font-bold uppercase tracking-widest mt-1">
          Real-time tracking of warehouse stocks and assets
        </p>
      </div>
      ...
      <Card className="border-none shadow-2xl bg-white dark:bg-gray-950 rounded-[2.5rem] overflow-hidden">
        ...
        <CardContent className="p-10">
          {loading ? (
            <div className="text-center py-20 animate-pulse font-black text-gray-300">
  ...
  return (
    <Card className="border-none shadow-xl bg-white dark:bg-gray-950 overflow-hidden">
      <CardContent className="p-6 flex items-center gap-6">
        <div
          className={`p-4 ${colors[color]} text-white rounded-3xl shadow-lg`}
        >
          {icon}
        </div>
        <div>
...
function InventoryCard({ item, onBuy }) {
  ...
  const handleBuy = async () => {
    if (buyQty < 1 || buyQty > item.quantity)
      ...
      <div className="p-6 bg-gray-50 dark:bg-gray-900 rounded-[2rem] border border-gray-100 dark:border-gray-800 flex flex-col justify-between gap-6 hover:shadow-lg transition-all group">
        <div className="flex justify-between items-start">
          <div className="flex items-center gap-4">
            <div
              className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black ${isLow ? "bg-rose-100 text-rose-600" : "bg-indigo-100 text-indigo-600"}`}
            >
              <Package size={20} />
            </div>
            <div>
          ...
          <span
            className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest ${isLow ? "bg-rose-100 text-rose-700" : "bg-emerald-100 text-emerald-700"}`}
          >
        ...
        <div className="flex items-center justify-between mt-2">
          ...
          <Button
            ...
            className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-widest flex items-center gap-2"
          >
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-gray-950 p-8 rounded-3xl w-full max-w-md shadow-2xl relative">
            <button
              ...
              className="absolute top-6 right-6 text-gray-400 hover:text-rose-500"
            >
              <X size={20} />
            </button>
            <h3 className="text-2xl font-black mb-2 flex items-center gap-2">
              <ShoppingCart className="text-indigo-600" /> Place Order
            </h3>
            <p className="text-xs text-gray-500 mb-6 font-bold uppercase tracking-widest">
              {item.name}
            </p>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl flex justify-between items-center">
                <span className="text-[10px] font-black uppercase tracking-widest text-indigo-800 dark:text-indigo-300">
                  Total Amount
                </span>
                <span className="text-xl font-black text-indigo-600 dark:text-indigo-400">
                  ₦{(buyQty * item.unitPrice).toLocaleString()}
                </span>
              </div>
            </div>
