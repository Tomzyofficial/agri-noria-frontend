...
/*********** Lucid icon***********/
import { LogOut, X, Menu, LayoutDashboard, Heart, ShoppingCart, Settings } from "lucide-react";
...
export default function DashboardLayout({ children }) {
   ...
   const navMenu = [
      { label: "Dashboard", href: "/buyer", icon: <LayoutDashboard className="w-4 h-4" /> },
      { label: "Orders", href: "/buyer/orders", icon: <ShoppingCart className="w-4 h-4" /> },
      { label: "Wishlist", href: "/buyer/wishlist", icon: <Heart className="w-4 h-4" /> },
      { label: "Settings", href: "/buyer/settings", icon: <Settings className="w-4 h-4" /> },
   ];

   const navLinksStyle = (path) => {
      return path === pathname
         ? "bg-(--gray-color) dark:bg-(--card-dark) p-2 rounded"
         : "hover:bg-(--gray-color) dark:hover:bg-(--card-dark) dark:focus:bg-(--card-dark) p-2 focus:bg-(--gray-color) rounded transition delay-50 duration-150 ease-in-out";
   };
               ...
               >
                  {navMenu.map((item) => (
                     <Link
                        key={item.label}
                        href={item.href}
                        className={`${navLinksStyle(item.href)} flex items-center gap-2`}
                     >
                  ...
                  <Button
                     ...
                     type="submit"
                     className="cursor-pointer absolute bottom-4 flex items-center text-red-400"
                  >
                     <span>
