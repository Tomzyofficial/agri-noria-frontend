...
import { 
   Landmark, Users, Activity, CheckCircle2, 
   TrendingUp, Calendar, ArrowUpRight, ShieldCheck,
   Loader2, FileText, Plus
} from "lucide-react";
   ...
   return (
      <div className="space-y-8 animate-in fade-in duration-500">
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
               <h1 className="text-4xl font-black text-(--foreground) tracking-tight">Institution Dashboard</h1>
               <p className="text-gray-500 mt-2 text-lg font-medium">Ecosystem orchestration and financial governance center.</p>
            </div>
            <div className="flex gap-3">
               <Button variant="outline" className="rounded-2xl h-12 px-6 font-bold border-gray-200 dark:border-gray-800">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
               { label: "Active Programs", value: stats.overview.activePrograms, icon: <Landmark />, color: "blue", sub: "Currently running" },
               { label: "Enrolled Farmers", value: stats.overview.totalFarmers.toLocaleString(), icon: <Users />, color: "emerald", sub: "Across all interventions" },
               { label: "Total Deployed", value: `₦${stats.overview.totalDeployed.toLocaleString()}`, icon: <TrendingUp />, color: "purple", sub: "Approved financing" },
               { label: "Total Hectares", value: stats.overview.totalHectares.toLocaleString(), icon: <ShieldCheck />, color: "orange", sub: "Under cultivation" },
            ].map((stat, i) => (
               <Card key={i} className="border-none shadow-xl bg-white dark:bg-gray-950 overflow-hidden relative group transition-all hover:-translate-y-1">
                  ...
                  <CardContent className="p-6">
                     ...
                     <p className="text-3xl font-black mt-1 text-gray-900 dark:text-gray-100">{stat.value}</p>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <Card className="lg:col-span-2 border-none shadow-xl bg-white dark:bg-gray-950">
               <CardHeader className="flex flex-row items-center justify-between border-b border-gray-100 dark:border-gray-800">
                  ...
                  <Link href="/dashboard/institution/programs" className="text-blue-600 text-sm font-bold hover:underline">View All</Link>
               </CardHeader>
               <CardContent className="p-0">
                  <div className="divide-y divide-gray-100 dark:divide-gray-800">
                     {stats.recentPrograms.map((prog, i) => (
                        <div key={i} className="p-6 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                           <div className="flex items-center gap-4">
                              ...
                              <div>
                                 <p className="font-bold text-gray-900 dark:text-gray-100">{prog.name}</p>
                                 <p className="text-xs text-gray-500">{prog.commodity} — {prog.region}</p>
                              </div>
                           </div>
                           <div className="text-right">
                              <p className="text-sm font-bold">{prog.target_farmers} Farmers</p>
                              <p className="text-[10px] text-gray-400 uppercase font-black">{new Date(prog.created_at).toLocaleDateString()}</p>
                           </div>
                        </div>
                     ))}
                     {stats.recentPrograms.length === 0 && (
                        <div className="p-12 text-center text-gray-400 italic">No programs launched yet.</div>
                     )}
                  </div>
               </CardContent>
            </Card>
                     ...
                     {stats.systemHealth.map((svc) => (
                        <div key={svc.id} className="flex items-center justify-between">
                           ...
                           <div className="flex items-center gap-2">
                              ...
                              <span className={`text-[10px] font-bold uppercase text-${svc.color}-600`}>{svc.status}</span>
                           </div>
                        </div>
                     ))}
