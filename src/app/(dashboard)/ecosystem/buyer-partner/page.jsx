    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 font-bold uppercase tracking-widest mt-1">
          Manage ecosystem orders, track logistics, and coordinate with
          clusters.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          label="Total Produce"
          value={`${totalTons.toLocaleString()} Tons`}
          ...
          <div className="flex flex-col md:flex-row items-end justify-between gap-6">
            <div>
              ...
              <p className="text-xs font-bold text-gray-400 mt-2 uppercase tracking-widest">
                Across all settled transactions via Paystack escrow
              </p>
            </div>
            <div className="bg-indigo-50 dark:bg-indigo-900/20 px-8 py-4 rounded-3xl border border-indigo-100 dark:border-indigo-800">
              ...
              <p className="text-xl font-black">
                ₦{((stats.totalSalesValue || 0) * 0.05).toLocaleString()}
              </p>
            </div>
          </div>
        </CardContent>
                ...
                >
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                      {match.commodity} • {match.quantity_tons} Tons
                    </p>
                  </div>
                  <span className="text-xs font-black text-indigo-600">
                    ₦{parseFloat(match.offer_price || 0).toLocaleString()}
                  </span>
                </div>
              ))}
            </div>
              ...
              {logistics.slice(0, 3).map((entry, i) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-black text-sm truncate max-w-[120px]">
                        {entry.warehouse_name || "Warehouse"}
                      </p>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                        {entry.status}
                      </p>
                  ...
                  <span className="text-[10px] font-black uppercase px-3 py-1 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-full">
                    {entry.weight_tons} T
                  </span>
                </div>
              ))}
