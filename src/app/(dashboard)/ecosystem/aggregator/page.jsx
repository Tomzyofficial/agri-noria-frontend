...
export default function AggregatorDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [profileRes, agreementsRes, walletRes, inputsRes, marketRes] =
          await Promise.all([
        ...
        if (agreementsRes.ok)
          setAgreements((await agreementsRes.json()).data || []);
        ...
        if (inputsRes?.ok)
          setInputRequests((await inputsRes.json()).data || []);
        if (marketRes?.ok)
          setMarketplaceData((await marketRes.json()).data || []);
      } catch (err) {
    ...
    <div className="space-y-8 animate-in fade-in duration-500 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-gray-800 p-8 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700">
        <div>
          <h1 className="text-4xl font-black tracking-tight text-gray-900 dark:text-white">
            ...
            <span className="text-green-600">
              {profile?.company_name || "Aggregator"}
            </span>
          </h1>
        ...
        <div className="flex gap-3">
          <Button
            onClick={() =>
              (window.location.href = "/ecosystem/aggregator/buyers")
            }
            className="bg-gray-900 text-white hover:bg-black px-6 py-6 rounded-2xl font-bold transition-all hover:scale-105"
          >
            Manage Buyers
          </Button>
          <Button
            onClick={() =>
              (window.location.href = "/ecosystem/aggregator/clusters")
            }
            className="bg-green-600 text-white hover:bg-green-700 px-6 py-6 rounded-2xl font-bold shadow-lg shadow-green-600/20 transition-all hover:scale-105"
          >
            View Clusters
          </Button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-4">
            {agreements.length === 0 ? (
              <p className="text-gray-500 italic text-center py-8">
                No recent procurement activities found.
              </p>
            ) : (
              agreements.slice(0, 5).map((ag) => (
                <div
                  key={ag.id}
                  className="group p-5 rounded-3xl border border-gray-50 dark:border-gray-800 hover:border-green-100 dark:hover:border-green-900 hover:bg-green-50/30 transition-all flex items-center justify-between"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">
                        {ag.product_details?.commodity || "General"} Procurement
                      </p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-black text-gray-900 dark:text-white">
                      ₦{parseFloat(ag.financing_amount).toLocaleString()}
                    </p>
                    <span
        ...
        <Card className="rounded-[2.5rem] border-none shadow-sm bg-gray-900 text-white p-8">
              ...
              >
                ...
                <div className="text-right">
                  <p className="font-black text-green-400">
                    ₦{parseFloat(m.current_price).toLocaleString()}
                  </p>
                  <p
                    className={`text-[10px] font-bold ${m.trend === "up" ? "text-green-500" : "text-red-500"}`}
                  >
                    {m.trend === "up" ? "▲ Up" : "▼ Down"}
                  </p>
                </div>
          ...
          <Button
            onClick={() =>
              (window.location.href = "/ecosystem/aggregator/marketplace")
            }
...
function StatCard({ title, value, icon, color, gradient }) {
  return (
    <Card
      className={`rounded-[2rem] border-none shadow-sm bg-gradient-to-br ${gradient} p-8`}
    >
